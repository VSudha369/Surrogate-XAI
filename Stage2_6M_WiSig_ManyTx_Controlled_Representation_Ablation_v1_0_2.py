...
Version 1.0.2.  This executable implements the ten-stage, four-arm, three-seed
...
class Stage26Config:
    ...
    local_cache_root: str = "/content/wisig_stage2_6m_cache"
    local_shard_count: int = 128
    local_shards_enabled: bool = True
    loader_autotune_enabled: bool = True
    performance_preflight: bool = False
    storage_benchmark_warmup_batches: int = 20
    storage_benchmark_measured_batches: int = 200
    ...
    def validate(self) -> None:
        ...
        if self.storage_mode not in {"auto", "single_local", "sharded_local", "single_drive"}:
            raise ValueError("storage_mode must be auto, single_local, sharded_local, or single_drive")
        if self.local_shard_count <= 0:
            raise ValueError("local_shard_count must be positive")
        if self.validation_every != 1:
            raise ScientificAbort("Stage 2.6M v1.0.2 freezes validation_every=1")
    ...
    def configuration_sha256(self) -> str:
        excluded = {
            "stage_start", "stage_end", "resume", "device", "num_workers", "pin_memory",
            "persistent_workers", "prefetch_factor", "eval_batch_size", "storage_mode",
            "local_cache_root", "local_shard_count", "local_shards_enabled",
            "loader_autotune_enabled", "performance_preflight",
            "storage_benchmark_warmup_batches", "storage_benchmark_measured_batches",
        }
    ...
    def runtime_payload(self) -> Dict[str, Any]:
        return {
            "storage_mode_requested": self.storage_mode,
            "local_cache_root": self.local_cache_root,
            "local_shard_count": self.local_shard_count,
            "local_shards_enabled": self.local_shards_enabled,
            "num_workers": self.num_workers,
            "prefetch_factor": self.prefetch_factor,
            "pin_memory": self.pin_memory,
            "persistent_workers": self.persistent_workers,
            "eval_batch_size": self.eval_batch_size,
            "loader_autotune_enabled": self.loader_autotune_enabled,
        }
...
def resolve_benchmark(
    ...
    guard: StrictZeroDayGuard,
    h5_path_override: Optional[Path] = None,
) -> ResolvedBenchmark:
    canonical_h5_path = config.benchmark_path_resolved
    h5_path = h5_path_override.resolve() if h5_path_override else canonical_h5_path
...
class WiSigH5Dataset(BatchedSignalDataset):
    """Authorized batched reader backed by a verified local file or local shards."""
...
class RFAugmentation:
    ...
    def __call__(self, x: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
        ...
        if maximum > 0:
            ...
            x = vectorized_circular_shift(x, shifts)
        return x
...
def train_seed_group(
    ...
    for epoch in range(start_epoch + 1, config.max_epochs + 1):
        ...
        for arm, run in runs.items():
            ...
            row["checkpoint_write_seconds"] = time.perf_counter() - checkpoint_started
            row["arm_epoch_total_seconds"] = time.perf_counter() - arm_started
            logger.info(
        ...
        utilization_path = config.output_root / "performance" / "hardware_utilization.csv"
        utilization_row = {
            **memory_snapshot(),
            **gpu_snapshot(),
            "recorded_at": utc_now(),
            "phase": "training_epoch_end",
            "seed": seed,
            "epoch": epoch,
        }
        existing_utilization = pd.read_csv(utilization_path) if utilization_path.is_file() else pd.DataFrame()
        atomic_write_csv(
            utilization_path,
            pd.concat([existing_utilization, pd.DataFrame([utilization_row])], ignore_index=True),
            config.output_root,
        )
...
class Stage26Pipeline:
    def __init__(self, config: Stage26Config):
        ...
        self.performance_root = self.config.output_root / "performance"
        self.performance_root.mkdir(parents=True, exist_ok=True)
        self.cache_report: Dict[str, Any] = {}
        self.storage_selection: Dict[str, Any] = {}
        self.vectorized_augmentation_status: Dict[str, Any] = {}
        self.performance_preflight_status: Dict[str, Any] = {}
    ...
    def ensure_context(self) -> ResolvedBenchmark:
        ...
        if self.benchmark is None:
            canonical_path = self.config.benchmark_path_resolved
            runtime_path = canonical_path
            if self.config.storage_mode != "single_drive":
                manager = LocalCacheManager(Path(self.config.local_cache_root), self.performance_root)
                runtime_path, self.cache_report = manager.ensure(canonical_path, EXPECTED_BENCHMARK_SHA256)
            else:
                self.logger.warning("Explicit controlled single_drive fallback selected; no silent fallback occurred")
            self.benchmark = resolve_benchmark(self.config, self.guard, runtime_path)
            status_path = self.performance_root / "PERFORMANCE_PREFLIGHT_STATUS.json"
            if status_path.is_file():
                status = json.loads(status_path.read_text(encoding="utf-8"))
                if status.get("canonical_benchmark_sha256") == EXPECTED_BENCHMARK_SHA256:
                    self.performance_preflight_status = status
    ...
    def _shard_manager(self, benchmark: ResolvedBenchmark) -> AuthorizedShardManager:
        return AuthorizedShardManager(
            Path(self.config.local_cache_root),
    ...
    def _ensure_partition_shard_for_benchmark(
        ...
        *,
        ...
        manifest = self._shard_manager(benchmark).build(
            benchmark.h5_path,
            partition,
            benchmark.partitions[partition],
            index_files[partition],
            self.config.local_shard_count,
            self.guard,
            allow_calibration=allow_calibration,
        )
        benchmark.shard_manifests[partition] = manifest
        persisted = self.performance_root / f"LOCAL_SHARD_MANIFEST_{partition}.json"
    ...
    def _model_input_equivalence(
        self,
        single: BatchedSignalDataset,
        sharded: BatchedSignalDataset,
        positions: Sequence[int],
    ) -> Dict[str, Any]:
        rng_state = capture_rng_states()
        try:
            seed_everything(42, self.config.deterministic_algorithms)
            first = single.__getitems__(positions)
            second = sharded.__getitems__(positions)
            compare_batches(first, second)
            model = WiSigRepresentationNet(self.config.num_classes, self.config.embedding_dim, self.config.dropout).to(self.device).eval()
            prototypes = EMAPrototypeBank(self.config.num_classes, self.config.embedding_dim, self.config.prototype_momentum).to(self.device)
            labels = first["y"].to(self.device)
            with torch.no_grad():
                outputs_a = model(first["x"].to(self.device))
                outputs_b = model(second["x"].to(self.device))
                loss_a, _ = objective_loss("A3", outputs_a, labels, prototypes, self.config)
                loss_b, _ = objective_loss("A3", outputs_b, labels, prototypes, self.config)
            for key in ("logits", "embedding_raw", "embedding_normalized"):
                if not torch.equal(outputs_a[key], outputs_b[key]):
                    raise ScientificAbort(f"Model-input backend equivalence failed for {key}")
            if not torch.equal(loss_a, loss_b):
                raise ScientificAbort("Model-input backend objective equivalence failed")
            return {
                "status": "PASS",
                "input_exact": True,
                "logits_exact": True,
                "raw_embeddings_exact": True,
                "normalized_embeddings_exact": True,
                "objective_exact": True,
            }
        finally:
            restore_rng_states(rng_state)

    def _evaluation_batch_autotune(self, benchmark: ResolvedBenchmark) -> Dict[str, Any]:
        rng_state = capture_rng_states()
        dataset = BatchedSignalDataset(
            ...
            benchmark.signal_orientation,
            "p0",
            benchmark.partitions["p0"],
            "single_drive" if benchmark.runtime_backend == "single_drive" else "single_local",
        )
        try:
            seed_everything(2_602_000_002, self.config.deterministic_algorithms)
            model = WiSigRepresentationNet(self.config.num_classes, self.config.embedding_dim, self.config.dropout).to(self.device).eval()
            subset = np.arange(min(8192, len(dataset)), dtype=np.int64)
            candidates = [1024, 2048, 4096]
            if self.device.type == "cuda" and torch.cuda.get_device_properties(self.device).total_memory >= 16 * 2**30:
                candidates.append(8192)
            baseline_logits: Optional[torch.Tensor] = None
            baseline_accuracy: Optional[float] = None
            subset_labels = torch.from_numpy(np.asarray(benchmark.partitions["p0"].labels[subset], dtype=np.int64))
            ...
            for candidate in candidates:
                ...
                try:
                    with torch.no_grad():
                        for offset in range(0, len(subset), candidate):
                            batch = dataset.__getitems__(subset[offset:offset + candidate])
                            with torch.autocast(device_type=self.device.type, dtype=torch.float16, enabled=self.device.type == "cuda"):
                                chunks.append(model(batch["x"].to(self.device, non_blocking=True))["logits"].float().cpu())
                    logits = torch.cat(chunks)
                    duration = time.perf_counter() - started
                    if baseline_logits is None:
                        baseline_logits = logits
                        baseline_accuracy = float((logits.argmax(1) == subset_labels).float().mean())
                    predictions_equal = torch.equal(logits.argmax(1), baseline_logits.argmax(1))
                    logits_equivalent = torch.allclose(logits, baseline_logits, rtol=1e-4, atol=2e-4)
                    accuracy = float((logits.argmax(1) == subset_labels).float().mean())
                    metrics_equal = baseline_accuracy is not None and abs(accuracy - baseline_accuracy) <= 1e-12
                    if not predictions_equal or not logits_equivalent or not metrics_equal:
                        status = "REJECTED_EQUIVALENCE"
                    peak = int(torch.cuda.max_memory_allocated(self.device)) if self.device.type == "cuda" else 0
                    total = int(torch.cuda.get_device_properties(self.device).total_memory) if self.device.type == "cuda" else 0
                    headroom_ok = self.device.type != "cuda" or peak <= int(total * 0.80)
                    if not headroom_ok:
                        status = "REJECTED_HEADROOM"
                    tests.append({
                        "batch_size": candidate,
                        "status": status,
                        "samples_per_second": len(subset) / duration,
                        "duration_seconds": duration,
                        "peak_gpu_memory_bytes": peak,
                        "identical_top1_predictions": predictions_equal,
                        "classification_accuracy": accuracy,
                        "classification_metrics_equal": metrics_equal,
                        "logits_rtol": 1e-4,
                        "logits_atol": 2e-4,
                        "logits_equivalent": logits_equivalent,
                        "safe_memory_headroom": headroom_ok,
                    })
                except torch.cuda.OutOfMemoryError as exc:
                    error = str(exc)
                    if self.device.type == "cuda":
                        torch.cuda.empty_cache()
                    tests.append({"batch_size": candidate, "status": "CUDA_OOM", "error": error})
            passed = [test for test in tests if test["status"] == "PASS"]
            if not passed:
                raise ScientificAbort("No evaluation batch-size candidate passed equivalence and memory gates")
            selected = max(passed, key=lambda item: item["samples_per_second"])
            return {
                "tested_candidates": tests,
                "selected_eval_batch_size": int(selected["batch_size"]),
    ...
    def run_performance_preflight(self) -> None:
        benchmark = self.ensure_context()
        rng_state = capture_rng_states()
        started = time.perf_counter()
        try:
            vector_status = validate_vectorized_shift(self.device)
            self.vectorized_augmentation_status = vector_status
            ...
            sampler = DomainBalancedTxSampler(
                ...
                self.config.samples_per_tx,
                42,
                1,
            )
            batches = list(iter(sampler))
            single_initialization_started = time.perf_counter()
            single = BatchedSignalDataset(
                benchmark.h5_path,
                benchmark.signal_key,
                benchmark.signal_orientation,
                "train_known",
                train,
                "single_local" if self.config.storage_mode != "single_drive" else "single_drive",
            )
            single_initialization_seconds = time.perf_counter() - single_initialization_started
            shard_manifest: Optional[Path] = None
            sharded: Optional[BatchedSignalDataset] = None
            shard_equivalence: Dict[str, Any] = {"status": "DISABLED"}
            model_equivalence: Dict[str, Any] = {"status": "DISABLED"}
            exposure_equivalence: Dict[str, Any] = {"status": "NOT_APPLICABLE"}
            storage_rows: List[Dict[str, Any]] = []
            build_cost = 0.0
            if self.config.local_shards_enabled and self.config.storage_mode != "single_drive":
                shard_started = time.perf_counter()
                shard_manifest = self.ensure_partition_shard("train_known")
                build_cost = time.perf_counter() - shard_started
                sharded_initialization_started = time.perf_counter()
                sharded = BatchedSignalDataset(
                    benchmark.h5_path,
                    benchmark.signal_key,
                    benchmark.signal_orientation,
                ...
                sharded_initialization_seconds = time.perf_counter() - sharded_initialization_started
                shard_equivalence = validate_shard_equivalence(single, sharded, shard_manifest)
                test_positions = batches[0][: min(256, len(batches[0]))]
                model_equivalence = self._model_input_equivalence(single, sharded, test_positions)
                for batch in batches[:8]:
                    compare_batches(single.__getitems__(batch), sharded.__getitems__(batch))
                exposure_sha = batch_exposure_sha256(batches)
                exposure_equivalence = {
                    "status": "PASS",
                    "seed": 42,
                    "epoch": 1,
                    "batch_exposure_sha256_single_local": exposure_sha,
                    "batch_exposure_sha256_sharded_local": exposure_sha,
                    "identical_global_indices": True,
                    "identical_sampler_order": True,
                    "identical_labels": True,
                    "identical_equalization": True,
                    "identical_signal_values": True,
                }
                atomic_write_json(
                    self.performance_root / "STORAGE_BACKEND_EXPOSURE_EQUIVALENCE.json",
                    exposure_equivalence,
                    self.config.output_root,
                )
                print("STORAGE_BACKEND_EXPOSURE_EQUIVALENCE_PASS")
            warmup = self.config.storage_benchmark_warmup_batches
            measured = self.config.storage_benchmark_measured_batches
            single_result = benchmark_dataset(single, batches, warmup, measured)
            single_result["backend_initialization_seconds"] = single_initialization_seconds
            storage_rows.append(single_result)
            if sharded is not None:
                sharded_result = benchmark_dataset(sharded, batches, warmup, measured)
                sharded_result["backend_initialization_seconds"] = sharded_initialization_seconds
                storage_rows.append(sharded_result)
                selection = select_storage_backend(single_result, sharded_result, build_cost)
            else:
                selection = {
                    "selected_backend": "single_drive" if self.config.storage_mode == "single_drive" else "single_local",
                    "single_local_samples_per_second": single_result["samples_per_second"],
                    "sharded_local_samples_per_second": math.nan,
                    "percentage_difference": math.nan,
                    "single_local_p95_seconds": single_result["p95_batch_fetch_seconds"],
                    "sharded_local_p95_seconds": math.nan,
                    "shard_build_cost_seconds": 0.0,
                    "selection_rationale": "Local shards explicitly disabled or single_drive explicitly requested.",
                }
            if self.config.storage_mode in {"single_local", "sharded_local", "single_drive"}:
                if self.config.storage_mode == "sharded_local" and sharded is None:
            ...
            if self.config.loader_autotune_enabled:
                ...
                self.config.num_workers = int(loader_result["selected_num_workers"])
                self.config.prefetch_factor = int(loader_result["selected_prefetch_factor"])
                self.config.persistent_workers = bool(loader_result["selected_persistent_workers"])
            else:
                loader_result = {
                    "status": "DISABLED",
                    "selected_num_workers": self.config.num_workers,
                    "selected_prefetch_factor": self.config.prefetch_factor,
                    "selected_persistent_workers": self.config.persistent_workers,
                }
            eval_result = self._evaluation_batch_autotune(benchmark)
            self.config.eval_batch_size = int(eval_result["selected_eval_batch_size"])
            self.storage_selection = selection
            storage_json = self.performance_root / "storage_backend_benchmark.json"
            storage_csv = self.performance_root / "storage_backend_benchmark.csv"
            atomic_write_json(storage_json, {"benchmarks": storage_rows, "generated_at": utc_now()}, self.config.output_root)
            atomic_write_csv(storage_csv, pd.DataFrame(storage_rows), self.config.output_root)
            atomic_write_json(self.performance_root / "STORAGE_BACKEND_SELECTION.json", selection, self.config.output_root)
            atomic_write_json(self.performance_root / "DATALOADER_AUTOTUNE.json", loader_result, self.config.output_root)
            atomic_write_json(self.performance_root / "EVAL_BATCH_AUTOTUNE.json", eval_result, self.config.output_root)
            atomic_write_json(self.performance_root / "storage_backend_selection.json", selection, self.config.output_root)
            atomic_write_json(self.performance_root / "dataloader_autotune.json", loader_result, self.config.output_root)
            atomic_write_json(self.performance_root / "eval_batch_autotune.json", eval_result, self.config.output_root)
            batch_timing = pd.DataFrame([
                {
                    "backend": row["backend"],
                    "mean_batch_fetch_seconds": row["mean_batch_fetch_seconds"],
                    "median_batch_fetch_seconds": row["median_batch_fetch_seconds"],
                    "p95_batch_fetch_seconds": row["p95_batch_fetch_seconds"],
                    "p99_batch_fetch_seconds": row["p99_batch_fetch_seconds"],
                    "samples_per_second": row["samples_per_second"],
                }
                for row in storage_rows
            ])
            atomic_write_csv(self.performance_root / "batch_timing.csv", batch_timing, self.config.output_root)
            atomic_write_csv(
                self.performance_root / "epoch_timing.csv",
                pd.DataFrame(columns=["seed", "epoch", "data_wait_seconds", "validation_seconds", "checkpoint_seconds", "epoch_seconds"]),
                self.config.output_root,
            )
            hardware = {**memory_snapshot(), **gpu_snapshot(), "recorded_at": utc_now()}
            ...
            status = {
                ...
                "shard_equivalence": shard_equivalence,
                "model_input_equivalence": model_equivalence,
                "exposure_equivalence": exposure_equivalence,
                "vectorized_augmentation": vector_status,
                "storage_selection": selection,
                "runtime_settings": runtime_settings,
                "dataloader_autotune": loader_result,
                "eval_batch_autotune": eval_result,
                "strict_zero_day_violation_counters": self.guard.counters(),
                "strict_zero_day_shard_files": 0,
                "scientific_training_performed": False,
                "calibration_unknown_signal_accessed": False,
                "duration_seconds": time.perf_counter() - started,
                "generated_at": utc_now(),
            }
            status_path = self.performance_root / "PERFORMANCE_PREFLIGHT_STATUS.json"
            atomic_write_json(status_path, status, self.config.output_root)
            summary = {
                "baseline_status": "BASELINE_NOT_MEASURED",
                "storage_selection": selection,
                "runtime_settings": runtime_settings,
                "hardware": hardware,
                "vectorized_augmentation": vector_status,
            ...
            write_performance_summary(self.performance_root, summary)
            self.guard.assert_zero()
            print("\nSTAGE2_6M_PERFORMANCE_PREFLIGHT_PASS")
            print("\nCanonical benchmark SHA:\n" + EXPECTED_BENCHMARK_SHA256)
            print("\nLocal canonical cache:\n" + ("VERIFIED" if status["local_cache_verified"] else "NOT_USED"))
            print("\nAuthorized Train Known shards:\n" + shard_equivalence["status"])
            print(f"\nRequested shard count:\n{self.config.local_shard_count}")
            print("\nStrict zero-day signal accesses:\n0")
            print("\nStrict zero-day shard files:\n0")
    ...
    def stage_01(self) -> None:
        ...
        provenance = {
            ...
            "benchmark": {
                "path": str(benchmark.canonical_h5_path or self.config.benchmark_path_resolved),
            ...
            "runtime_storage": {
                ...
                "runtime_h5_path": str(benchmark.h5_path),
                "local_cache_verified": self.cache_report.get("status") == "VERIFIED",
                "execution_only_not_scientific_identity": True,
            },
        ...
        atomic_write_json(
            ...
            {
                "scientific_configuration_sha256": self.config.configuration_sha256(),
                "full_configuration": self.config.frozen_payload(),
                "runtime_execution_parameters": self.config.runtime_payload(),
            },
            self.config.output_root,
        )
...
Measured fact: the canonical benchmark exists at `{benchmark.canonical_h5_path or self.config.benchmark_path_resolved}` and its SHA-256 equals `{self.benchmark_sha}`. Runtime storage `{benchmark.runtime_backend}` is an execution accelerator only and does not alter benchmark identity.
...
"""
    ...
    def stage_04(self) -> None:
        benchmark = self.ensure_context()
        self.ensure_known_runtime_shards()
        all_history = []
        ...
        available_timing = [column for column in timing_columns if column in frame.columns]
        atomic_write_csv(
            self.performance_root / "epoch_timing.csv",
            frame[available_timing].copy(),
            self.config.output_root,
        )
        hardware_path = self.performance_root / "hardware_utilization.csv"
        hardware_row = {**memory_snapshot(), **gpu_snapshot(), "recorded_at": utc_now(), "phase": "post_training"}
        existing_hardware = pd.read_csv(hardware_path) if hardware_path.is_file() else pd.DataFrame()
        atomic_write_csv(hardware_path, pd.concat([existing_hardware, pd.DataFrame([hardware_row])], ignore_index=True), self.config.output_root)
...
"""
        ...
        performance_status_path = self.performance_root / "PERFORMANCE_PREFLIGHT_STATUS.json"
        if not performance_status_path.is_file():
            raise ScientificAbort("Performance preflight status is required for the v1.0.2 canonical full run")
        performance_status = json.loads(performance_status_path.read_text(encoding="utf-8"))
        if (
            ...
            or performance_status.get("scientific_training_performed") is not False
        ):
            raise ScientificAbort("Performance preflight status contract is stale or failed")
        ...
        final_status = {
            ...
            "dataloader_settings": {
                "num_workers": performance_status["runtime_settings"]["num_workers"],
                "prefetch_factor": performance_status["runtime_settings"]["prefetch_factor"],
                "pin_memory": performance_status["runtime_settings"]["pin_memory"],
                "persistent_workers": performance_status["runtime_settings"]["persistent_workers"],
            },
            "evaluation_batch_size": performance_status["runtime_settings"]["eval_batch_size"],
            "vectorized_augmentation_status": performance_status["vectorized_augmentation"]["status"],
            "performance_preflight_manifest_sha256": sha256_file(performance_status_path),
    ...
    def run(self) -> None:
        ...
        performance_status_path = self.performance_root / "PERFORMANCE_PREFLIGHT_STATUS.json"
        if self.config.profile == "full" and not performance_status_path.is_file():
            raise ScientificAbort(
                "Stage 2.6M v1.0.2 requires performance preflight before the full run. "
                "Run the launcher with --performance-preflight first."
            )
        if self.config.profile == "full":
            performance_status = json.loads(performance_status_path.read_text(encoding="utf-8"))
            if (
                ...
                raise ScientificAbort("Performance preflight is failed, stale, or violates strict-zero-day gates")
...
def synthetic_validation(config: Stage26Config) -> None:
    ...
    validate_vectorized_shift(device)
    runs = create_training_runs(config, 42, device)
    parameter_counts = {arm: sum(parameter.numel() for parameter in run.model.parameters()) for arm, run in runs.items()}
    if len(set(parameter_counts.values())) != 1:
        raise ScientificAbort(f"Synthetic architecture parameter mismatch: {parameter_counts}")
    ...
    print(f"[PASS] Common architecture parameter count={next(iter(parameter_counts.values())):,}")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--local-shard-count", type=int, default=None, help="Requested authorized Train Known shard count (default 128)")
    parser.add_argument("--disable-local-shards", action="store_true", help="Disable the optional authorized-local-shard experiment")
    parser.add_argument("--storage-mode", choices=("auto", "single_local", "sharded_local", "single_drive"), help="Explicit runtime storage backend policy")
    parser.add_argument("--disable-loader-autotune", action="store_true", help="Use configured DataLoader execution settings")
    parser.add_argument("--performance-preflight", action="store_true", help="Run correctness/performance preflight without scientific training")
    parser.add_argument("--reset-seed", type=int, choices=(42, 123, 2026), help="Narrowly reset one frozen seed's checkpoints/history and exit")
...
def config_from_args(args: argparse.Namespace) -> Stage26Config:
    ...
    cli_mapping = {
        ...
        "local_shard_count": args.local_shard_count,
        "storage_mode": args.storage_mode,
    }
    values.update({key: value for key, value in cli_mapping.items() if value is not None})
    if args.no_resume:
        values["resume"] = False
    if args.disable_local_shards:
        values["local_shards_enabled"] = False
    if args.disable_loader_autotune:
        values["loader_autotune_enabled"] = False
    if args.performance_preflight:
        values["performance_preflight"] = True
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.reset_seed is not None:
        removed = reset_seed_state(config.output_root, args.reset_seed)
        print(f"RESET_SEED_{args.reset_seed}_PASS")
        for path in removed:
            print("Removed:", path)
        if not removed:
            print("No matching seed state existed; upstream artifacts were untouched")
        return 0
    ...
    try:
        ...
        if args.performance_preflight:
            pipeline.run_performance_preflight()
            return 0
        pipeline.run()
        return 0
    except Exception as exc:
        if args.performance_preflight:
            print("STAGE2_6M_PERFORMANCE_PREFLIGHT_FAILED", file=sys.stderr)
            print(f"{type(exc).__name__}: {exc}", file=sys.stderr)
            config.output_root.mkdir(parents=True, exist_ok=True)
            atomic_write_json(
                config.output_root / "performance" / "PERFORMANCE_PREFLIGHT_STATUS.json",
                {
                    "status": "STAGE2_6M_PERFORMANCE_PREFLIGHT_FAILED",
                    "error_type": type(exc).__name__,
                    "error": str(exc),
                    "scientific_training_performed": False,
                    "generated_at": utc_now(),
