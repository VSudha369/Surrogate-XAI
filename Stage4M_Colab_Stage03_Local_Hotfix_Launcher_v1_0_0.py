#!/usr/bin/env python3
"""Colab launcher for the Stage 4M Stage-03 local-SSD activation hotfix."""
...
HOTFIX_EXECUTABLE = "Stage4M_Stage03_Local_Activation_Hotfix_v1_0_0.py"
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print("=" * 100)
    print("STAGE 4M — COLAB LOCAL-SSD HOTFIX LAUNCHER v1.0.0")
    print("=" * 100)
    mount_drive()
    root = discover(args.branch_root)
    repository = Path(args.repository_root).expanduser().resolve()
    pipeline = repository / HOTFIX_EXECUTABLE
    if not pipeline.is_file():
        raise RuntimeError(f"Stage 4M hotfix executable missing: {pipeline}")
    print(f"Branch root: {root}")
    print(f"Hotfix executable: {pipeline}")
