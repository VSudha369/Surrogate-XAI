#!/usr/bin/env python3
"""Colab launcher for canonical Stage 4M v1.0.0."""
...
def mount_drive() -> None:
    ...
    if not mount.is_dir(): raise RuntimeError("Google Drive mount did not become accessible")


def valid_root(path: Path) -> bool:
    required = ("01_benchmark_engineering", "02_benchmark_diagnostics", "03_representation_ablation", "04_canonical_teacher", "05_zero_day_open_set")
    return all((path / name).is_dir() for name in required) and (path / "04_canonical_teacher" / "MANYTX_STAGE3M_READY.txt").is_file() and (path / "05_zero_day_open_set" / "MANYTX_STAGE3_5M_READY.txt").is_file()


def discover(explicit: Optional[str]) -> Path:
    requested = explicit or os.environ.get("WISIG_BRANCH_ROOT")
    candidates = [Path(requested).expanduser().resolve()] if requested else [path.resolve() for path in Path("/content/drive/MyDrive").rglob(CANONICAL_BRANCH) if path.is_dir()]
    valid = [path for path in candidates if valid_root(path)]
    if not valid: raise RuntimeError("No valid canonical branch root found; verify the mounted Google account")
    if len(valid) != 1: raise RuntimeError("Multiple canonical branch roots found:\n" + "\n".join(map(str, valid)))
    return valid[0]
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print("=" * 100 + "\nSTAGE 4M — COLAB LAUNCHER v1.0.0\n" + "=" * 100)
    mount_drive(); root = discover(args.branch_root)
    repository = Path(args.repository_root).expanduser().resolve()
    pipeline = repository / "Stage4M_WiSig_ManyTx_Surrogate_KD_v1_0_0.py"
    if not pipeline.is_file(): raise RuntimeError(f"Stage 4M pipeline missing: {pipeline}")
    print(f"Branch root: {root}\nPipeline script: {pipeline}")
    if __import__("torch").cuda.is_available():
        torch = __import__("torch"); print(f"GPU: {torch.cuda.get_device_name(0)}\nGPU memory GiB: {torch.cuda.get_device_properties(0).total_memory / 2**30:.2f}")
