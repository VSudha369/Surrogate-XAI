...
`FINAL_STRICT_SCORE_BUNDLE.json` additionally binds each strict store manifest, score array, prediction array, global-index file, global-index value hash, sealed-index SHA, both recovery audits, and the recovery manifest. It explicitly declares the shifted subset, absence of labels, absence of signal reinference, and absence of score recomputation. `statistics/STRICT_BOOTSTRAP_PROVENANCE.json` records the fixed base seed and independently derived partition/scorer stream seeds.

`MANYTX_STAGE3_5M_READY.txt` is created only after the final status and self-consistent hash manifest. The hash manifest explicitly excludes itself, the Stage-11 checkpoint, and READY/NOT_READY markers to avoid recursion. READY is atomically written and byte-verified, then the Stage-11 checkpoint is written and hash-verified as the durable commit record. Only after that commit is valid may the historical NOT_READY marker be removed. A temporary READY+NOT_READY state is recoverable; READY without either a valid Stage-11 checkpoint or the reviewed NOT_READY marker is rejected.
